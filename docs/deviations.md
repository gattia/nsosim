...
mode Y; to get mode Z, do W."** The docstrings and [Coordinate systems &
pipeline](coordinate-systems.md) describe *what the code does*; this page is the only place
that frames it as a gap and points at the lever.
...
`reg_mode='similarity'`) divides out the subject's true anatomical scale; tibia/patella
reuse the femur transform. The MRI path to OSIM uses only the underscore
[`convert_nsm_recon_to_OSIM_`][nsosim.nsm_fitting.convert_nsm_recon_to_OSIM_], which adds
the fixed ref-center, converts mm→m, and axis-swaps — with **no subject-scale step**. So
every built knee is ~reference size: shape-personalized, size-normalized.
...
`scale` term into [`undo_transform`][nsosim.nsm_fitting.undo_transform]; a non-unit `scale`
re-introduces true size. The synthetic path already exercises this converter (with
`scale=1`). A keep-true-size ("Pathway C") mode would route the MRI recon through this hook
with the subject's true scale instead of the underscore converter. Not built.
...
([`bake_knee_geometry`][nsosim.scaling.knee_geometry.bake_knee_geometry]). But the knee
build then writes the **reference-scale recon** (`femur_nsm_recon_osim.stl`, …) and
repoints the model to it
...
`scale_factors` left at `1,1,1`. The recon is never multiplied by `s_wa`. Result: a
reference-scale knee inside an `s_wa`-scaled body — a geometric mismatch.
...
**common center = body-local origin = knee joint center** (see [Coordinate systems &
pipeline §1, §5](coordinate-systems.md)) at the OSIM-entry point
([`nsm_recon_to_osim`][nsosim.nsm_fitting.nsm_recon_to_osim] /
[`convert_nsm_recon_to_OSIM_`][nsosim.nsm_fitting.convert_nsm_recon_to_OSIM_]), and equally
to the warp-path attachment converter, the labeled mesh fed to the wrap fitter, and
`mean_patella` (deviation §3). Not built.
...
subtracts the patella's mean position (`mean_patella`) so the patella STL is centered at
its body-local origin; that offset is later written as the patellofemoral joint coordinate
translation (via [`update_osim_model`][nsosim.comak_osim_update.update_osim_model]).
`mean_patella` is a translation in **OSIM meters at reference size**.
...
([`bake_knee_geometry`][nsosim.scaling.knee_geometry.bake_knee_geometry]). The knee build
then repoints the model to the **differently named** recon STLs
...
[`save_geometry_files`][nsosim.model_building.save_geometry_files] copies new files and the
model is repointed. Net outcome matches the overwrite story, but the mechanism differs —
and the difference matters for a fix (you scale the recon that gets *repointed to*; the
Stage-X bone bake is dead weight under Pathway B).
...
- Apply `s_wa` to the knee recon about the common center at the OSIM-entry point
  ([`nsm_recon_to_osim`][nsosim.nsm_fitting.nsm_recon_to_osim] /
  [`convert_nsm_recon_to_OSIM_`][nsosim.nsm_fitting.convert_nsm_recon_to_OSIM_]), **and** to
  the warp-path attachment converter, **and** to the labeled mesh fed to the wrap fitter,
  **and** to `mean_patella`.
- Thread the synthetic path ([`nsosim.decode`](reference/decode.md)) separately if the
  feature is meant to be general.
