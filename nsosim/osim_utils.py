    ...
    for body_name, body_dict in dict_body_geometries_update.items():
        ...
        for geo_osim_idx in range(n_geo_osim):
            ...
            if name in body_dict.keys():
                ...
                logger.warning(
                    f'Geometry "{name}" not found in body_dict for body "{body_name}" - skipping'
                )
...
def update_contact_mesh_files(model, dict_contact_mesh_files_update):
    ...
    for contact_name, contact_dict in dict_contact_mesh_files_update.items():
        ...
        try:
            contact_geometry = osim.Smith2018ContactMesh.safeDownCast(
                contact_geometries.get(contact_name)
            )
        ...
        if contact_geometry is None and contact_name in FALLBACK_NAMES:
            for fallback_name in FALLBACK_NAMES[contact_name]:
                try:
                    contact_geometry = osim.Smith2018ContactMesh.safeDownCast(
                        contact_geometries.get(fallback_name)
                    )
                    if contact_geometry is not None:
                        actual_name_used = fallback_name
                        logger.info(
                            f'Contact geometry "{contact_name}" not found, using fallback name "{fallback_name}"'
                        )
                        break
                except RuntimeError:
                    continue
        ...
        if contact_geometry is not None:
            ...
            logger.error(
                f'Contact geometry "{contact_name}" not found in model (tried fallbacks: {FALLBACK_NAMES.get(contact_name, [])})'
            )

    # Log summary and raise error if ANY updates failed
    logger.info(
        f"Contact geometry update summary: {updated_count}/{len(dict_contact_mesh_files_update)} successful"
    )

    if failed_updates:
        error_msg = f"Failed to update {len(failed_updates)} contact geometries:\n" + "\n".join(
            [f"  - {failure}" for failure in failed_updates]
        )
...
def create_articular_contact_force(
    ...
    logger.debug(
        f'Created articular contact force "{name}" between "{socket_target_mesh}" and "{socket_casting_mesh}"'
    )

    return contact_force


def add_contact_force_to_model(
    model: osim.Model, contact_force: osim.Smith2018ArticularContactForce
):
    """
    ...
    for joint_name, joint_dict in dict_joint_default_values_update.items():
        ...
        for coordinate_idx, coordinate_value in joint_dict.items():
            ...
            if incremental:
                ...
                logger.debug(
                    f"Updated joint {joint_name} coordinate {coordinate_idx}: {current_value} + {coordinate_value} = {new_value}"
                )
            else:
                # Replace existing default value
                coordinate.set_default_value(round(coordinate_value, ROUND_DIGITS))
                logger.debug(
                    f"Set joint {joint_name} coordinate {coordinate_idx} to {coordinate_value}"
                )
...
def express_point_in_frame(
    ...
    station_target = source_frame.findStationLocationInAnotherFrame(
        state, station_source, target_frame
    )
    ...
    for force_idx in range(forcesets.getSize()):
        ...
        if class_name in ["Millard2012EquilibriumMuscle", "Blankevoort1991Ligament"]:
            ...
            if force_.getConcreteClassName() == "Millard2012EquilibriumMuscle":
                ...
                dict_force_lengths[force_name]["reference_strain"] = round(
                    reference_strain, ROUND_DIGITS
                )
...
def update_reference_strains_from_attachments_dict(
    ref_force_info: dict, dict_lig_mus_attach: dict
) -> dict:
    """
    ...
    for force_name, force_dict in dict_lig_mus_attach.items():
        if "reference_strain" in force_dict and force_name in ref_force_info:
            logger.debug(
                f"Overriding reference strain for {force_name}: "
...
def update_model_attachments_slacks(
    ...
    ref_force_info = update_reference_strains_from_attachments_dict(
        ref_force_info=ref_force_info, dict_lig_mus_attach=dict_lig_mus_attach
    )
    ...
    update_slack_lengths(model=model, force_length_dict=ref_force_info, state=updated_state)
