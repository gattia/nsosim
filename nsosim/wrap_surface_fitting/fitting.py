                        ...
                        for param in parameters:
                            if param.shape == (4,):  # This is a quaternion parameter
                                ...
                                if lbfgs_step % 20 == 0:  # Log every 20 steps to avoid spam
                                    ...
                                    logger.debug(f"L-BFGS step {lbfgs_step}: quaternion norm before normalization: {quat_norm_before:.6f}")
                                ...
                                if lbfgs_step % 20 == 0:  # Log every 20 steps to avoid spam
                                    ...
                                    logger.debug(f"L-BFGS step {lbfgs_step}: quaternion norm after normalization: {quat_norm_after:.6f}")
                ...
                for param in parameters:
                    if param.shape == (4,):  # This is a quaternion parameter
                        ...
                        if abs(quat_norm - 1.0) > 1e-6:
                            logger.warning(f"Final quaternion norm is {quat_norm:.6f} (should be 1.0)")
                        
                        # Check rotation matrix validity
                        R = RotationUtils.rot_from_quat(param.data)
                        R_det = torch.det(R).item()
                        R_orthogonal = torch.allclose(R @ R.T, torch.eye(3, device=R.device), atol=1e-6)
                        
                        if abs(R_det - 1.0) > 1e-6:
                            logger.warning(f"Final rotation matrix determinant is {R_det:.6f} (should be 1.0)")
                        if not R_orthogonal:
                            logger.warning("Final rotation matrix is not orthogonal")
                        
                        logger.info(f"Final rotation validation: det={R_det:.6f}, orthogonal={R_orthogonal}, quat_norm={quat_norm:.6f}")
                        break
