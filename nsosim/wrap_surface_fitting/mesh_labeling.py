...
def label_mesh_vertices_for_wrap_surfaces(
    mesh_path: str,
    osim_path: str,
    bone_name: str,
    ...
    if bone_name not in bone_config:
        ...
        raise ValueError(
            f"Bone '{bone_name}' not found in configuration. " f"Available bones: {available_bones}"
        )
...
def label_multiple_meshes(
    geometry_folder: str,
    osim_path: str,
    bone_config: Optional[Dict] = None,
    near_surface_thresholds: Optional[Union[float, Dict[str, float]]] = None,
) -> Dict[str, Mesh]:
    """
...
def get_labeled_vertex_data(mesh: Mesh, surface_name: str) -> Dict[str, any]:
    ...
    if sdf_key not in available_keys:
        raise ValueError(
            f"SDF data '{sdf_key}' not found in mesh. " f"Available keys: {available_keys}"
        )

    if binary_key not in available_keys:
        raise ValueError(
            f"Binary data '{binary_key}' not found in mesh. " f"Available keys: {available_keys}"
        )
