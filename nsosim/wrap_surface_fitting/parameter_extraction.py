...
from .utils import (
    extract_wrap_parameters,
    create_ellipsoid_polydata,
    create_cylinder_polydata
)
from .config import DEFAULT_SMITH2019_BONES


def extract_wrap_parameters_from_osim(osim_file_path: str, 
                                    bone_config: Optional[Dict] = None) -> Dict:
    """
        ...
        for body_name, body_data in bone_data.items():
            ...
            for wrap_name, wrap_params_single in body_data.items():
                ...
                if surface_type == 'WrapEllipsoid':
                    ...
                    print(f"Warning: Unknown wrap surface type '{surface_type}' for {wrap_name}. Skipping.")
                    continue
...
def create_single_wrap_surface_mesh(wrap_params: Dict, 
                                  surface_type: Optional[str] = None) -> pv.PolyData:
    """
    ...
    if surface_type is None:
        ...
        if not surface_type:
            raise ValueError("Cannot determine surface type. Parameters must contain a 'type' field "
...
def extract_and_create_wrap_surfaces(osim_file_path: str,
                                   bone_config: Optional[Dict] = None) -> Dict:
    """
