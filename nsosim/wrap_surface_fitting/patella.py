...
def label_patella_within_wrap_extents(patella_mesh, wrap_surface_mesh):
    ...
    patella_points_within_x = ((patella_mesh.point_coords[:,0] >= min_x_ellipse) & (patella_mesh.point_coords[:,0] <= max_x_ellipse)).astype(int)
    patella_mesh['within_x_ellipse'] = patella_points_within_x

    # Label patella points within the y extent of the wrap
    patella_points_within_y = ((patella_mesh.point_coords[:,1] >= min_y_ellipse) & (patella_mesh.point_coords[:,1] <= max_y_ellipse)).astype(int)
    patella_mesh['within_y_ellipse'] = patella_points_within_y

    # Label patella points within the z extent of the wrap
    patella_points_within_z = ((patella_mesh.point_coords[:,2] >= min_z_ellipse) & (patella_mesh.point_coords[:,2] <= max_z_ellipse)).astype(int)
