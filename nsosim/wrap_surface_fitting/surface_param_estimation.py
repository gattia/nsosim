...
def get_near_surface_points_from_mesh(mesh, surface_name):
    ...
    if near_surface_key not in mesh.mesh.point_data:
        raise ValueError(
            f"Near-surface data '{near_surface_key}' not found in mesh. "
...
def estimate_cylinder_center_z_axis(near_surface_points, slice_thickness=1.0):
    ...
    z_centers = torch.arange(
        z_min, z_max + slice_thickness, slice_thickness, device=near_surface_points.device
    )
...
def fit_cylinder_geometric(near_surface_points, num_slices=20):
    ...
    try:
        ...
        if num_slices < 2:
            warnings.warn(
                f"num_slices is {num_slices}, which is less than 2. Geometric cylinder fitting requires at least 2 slices and will likely fallback."
            )
            # Force fallback by ensuring few/no valid slices.
            slice_thickness = (
                z_range * 2.0 if z_range > 1e-6 else 1.0
            )  # Make slice very thick or default
            ...
            warnings.warn(
                "Z-range of points is very small (<= 1e-6). Cylinder is likely flat or points are co-planar. Geometric fitting will likely fallback."
            )
            ...
            slice_thickness = (
                z_range / num_slices
            )  # This is the thickness of each of the num_slices segments
            # Generate centers of each slice
            z_centers = torch.linspace(
                z_min + slice_thickness / 2.0,
        ...
        z_axis = torch.tensor([0.0, 0.0, 1.0], device=points.device, dtype=points.dtype)
        ...
        return {
            ...
            "radius": radius,
            "half_length": half_length,
            "axis": torch.tensor([0.0, 0.0, 1.0], device=points.device, dtype=points.dtype),
...
def fit_ellipsoid_algebraic(points):
    ...
    D_matrix = torch.stack(
        [x * x, y * y, z * z, x * y, x * z, y * z, x, y, z, torch.ones_like(x)], dim=1
    )
    ...
    try:
        ...
        Q = torch.tensor(
            [[A_c, D_coeff_c / 2, E_c / 2], [D_coeff_c / 2, B_c, F_c / 2], [E_c / 2, F_c / 2, C_c]],
        ...
        linear_terms = torch.tensor(
            [G_c, H_c, I_c], device=points_tensor.device, dtype=points_tensor.dtype
        )
        ...
        if (-J_prime / eigenvals).min() < 0 and not torch.allclose(
            (-J_prime / eigenvals).min(), torch.tensor(0.0, dtype=points_tensor.dtype)
        ):
            warnings.warn(
                f"Non-positive value for axes squared: {-J_prime / eigenvals}. Might not be an ellipsoid. Values: {(-J_prime / eigenvals).tolist()}"
            )

        axes_squared_inv = eigenvals / (-J_prime)
        axes = 1.0 / torch.sqrt(
            torch.abs(axes_squared_inv) + 1e-9
