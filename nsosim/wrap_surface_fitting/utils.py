...
def convert_fitted_ellipsoid_to_wrap_params(center, axes=None, rotation_matrix=None):
    ...
    if axes is None and rotation_matrix is None:
        if isinstance(center, (tuple, list)) and len(center) == 3:
            ...
            raise ValueError("If axes and rotation_matrix are None, center must be a tuple of (center, axes, rotation_matrix)")
...
def convert_fitted_cylinder_to_wrap_params(center, radius_length=None, rotation_matrix=None):
    ...
    if radius_length is None and rotation_matrix is None:
        if isinstance(center, (tuple, list)) and len(center) == 3:
            ...
            raise ValueError("If radius_length and rotation_matrix are None, center must be a tuple of (center, radius_length, rotation_matrix)")
    ...
    radius = radius_length[0].item() if hasattr(radius_length[0], 'item') else float(radius_length[0])
    half_length = radius_length[1].item() if hasattr(radius_length[1], 'item') else float(radius_length[1])
                ...
                if surface_type == 'ellipsoid':
                    for surface_name in surface_names:
                        ...
                        if ellipsoid is not None:
                            xyz_rot = [float(x) for x in ellipsoid.find('xyz_body_rotation').text.split()]
                            trans = [float(x) - offset[idx] for idx, x in enumerate(ellipsoid.find('translation').text.split())]
                ...
                if surface_type == 'cylinder':
                    for surface_name in surface_names:
                        ...
                        if cylinder is not None:
                            xyz_rot = [float(x) for x in cylinder.find('xyz_body_rotation').text.split()]
                            trans = [float(x) - offset[idx] for idx, x in enumerate(cylinder.find('translation').text.split())]
...
def prepare_fitting_data(bone_mesh_path: str, xml_path: str, bone_name: str, bone_dict: Dict, near_surface_threshold: float = 0.005) -> Mesh:
    """
    ...
    if coord_range < 1.0:  # Likely in meters
        ...
        print(f"[INFO] Detected mesh coordinates in meters (range: {coord_range:.3f}m). Using threshold: {near_surface_threshold}m")
    else:  # Likely in mm
        scale_factor = 1.0
        print(f"[INFO] Detected mesh coordinates in mm (range: {coord_range:.1f}mm). Using threshold: {near_surface_threshold}mm")
...
def prepare_multi_bone_fitting_data(geometry_folder: str, xml_path: str, bone_dict: Dict, near_surface_threshold = 0.0005) -> Dict:
    """
    ...
    for bone_name, bone_data in bone_dict.items():
        ...
        mesh_with_data = prepare_fitting_data(bone_mesh_path, xml_path, bone_name, bone_dict, bone_threshold)
