...
def load_actual_reference_meshes():
    ...
    for bone, path in mesh_paths.items():
        if os.path.exists(path):
            mesh = Mesh(path)
            meshes[bone] = {"mesh": mesh, "n_points": mesh.point_coords.shape[0], "path": path}
...
def quick_interpolation_test(bone_name, mesh_data, n_steps=20):
    ...
    model_configs = {
        "femur": {
            "path_model_state": os.path.join(
                folder_nsm_models, "568_nsm_femur_bone_cart_men_v0.0.1", "model", "2000.pth"
            ),
            "path_model_config": os.path.join(
                folder_nsm_models, "568_nsm_femur_bone_cart_men_v0.0.1", "model_params_config.json"
            ),
        },
        "tibia": {
            "path_model_state": os.path.join(
                folder_nsm_models, "650_nsm_tibia_v0.0.1", "model", "2000.pth"
            ),
            "path_model_config": os.path.join(
                folder_nsm_models, "650_nsm_tibia_v0.0.1", "model_params_config.json"
            ),
        },
        "patella": {
            "path_model_state": os.path.join(
                folder_nsm_models, "648_nsm_patella_v0.0.1", "model", "2000.pth"
            ),
            "path_model_config": os.path.join(
                folder_nsm_models, "648_nsm_patella_v0.0.1", "model_params_config.json"
            ),
        },
    }
    ...
    try:
        ...
        fem_path_transform_file = os.path.join(
            folder_ref_recons, "femur", "ref_femur_alignment.json"
        )
...
def main():
    ...
    if results:
        ...
        for result in results:
            ...
            print(
                f"{result['bone']:<8} {result['n_points']:<8} {result['time']:<8.2f} {ratio:<8.1f}x"
            )
